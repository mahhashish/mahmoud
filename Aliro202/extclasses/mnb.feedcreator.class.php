...
3 May 2009 Martin Brampton (counterpoint)
	Added Aliro specific startup code and converted to PHP5
...
class UniversalFeedCreator {
	...
	// Constructor added specifically for Aliro
	public function __construct () {
		$rsstype = strtoupper(aliroRequest::getInstance()->getParam($_REQUEST, 'rsstype', 'ATOM'));
		$this->_setFormat($rsstype);
	}

	public function __call ($method, $args) {
		if (is_object($this->_feed)) call_user_func_array(array($this->_feed, $method), $args);
		else trigger_error (T_('Method called on Universal Feed Creator before instance set up'));
	}
	
	public function __set ($property, $value) {
		if (is_object($this->_feed)) $this->_feed->$property = $value;
		else trigger_error (T_('Property set on Universal Feed Creator before instance set up'));
	}
	
	public function __get ($property) {
		return isset($this->_feed->$property) ? $this->_feed->$property : null;
	}
	 ...
	 */
	public function saveFeed($format="", $filename="", $displayContents=true) {
		if ($format) $this->_setFormat($format);
		$filename = $filename ? $filename : _ALIRO_SITE_BASE.'/cache/rssfeeds/'.aliroRequest::getInstance()->getOption().'.'.$this->_format.'.xml';
    ...
    */
   public function useCached($format="", $filename="", $timeout=3600) {
      if ($format) $this->_setFormat($format);
...
class AtomCreator10 extends FeedCreator {
	...
	public function createFeed() {
		...
		$feed.= "    <link rel=\"alternate\" type=\"text/html\" hreflang=\"en\" href=\"".$this->link."\"/>\n";
		$feed.= "    <link rel=\"self\" type=\"application/atom+xml\"  hreflang=\"en\" href=\"".$this->selflink."\"/>\n";
		$feed.= "    <id>".htmlspecialchars($this->link)."/</id>\n";
