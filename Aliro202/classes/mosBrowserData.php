...
// This code is kept purely for reference, it is not used in Aliro
...
*/
class mosBrowserData {

	private $browserSearchOrder = array (
...
// browsers.
//---------------------------------------------------------------------------
private static $BrowsersHereAreGrabbers = array (
"teleport"=>"1",
"webcapture"=>"1",
"webcopier"=>"1",
);
...
// KHTML (Konqueror, Safari)
	...
	function browserSearch ($agent) {
		foreach ($this->browserSearchOrder as $key) {
			if (preg_match( "/$key.?\/([\d\.]*)/i", $agent, $m )) {
				$name = "$this->browsersAlias[$key] $m[1]";
				return $name;
			}
		}
		return 'Unknown';
