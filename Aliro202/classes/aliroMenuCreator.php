...
class aliroMenuCreator {
	...
	*/
	...
	public function createMenu ($params) {
		...
		$text .= '>';
		$lastofall = count($entries) - 1;
		for ($i = $lastofall; $i >= 0; $i--) {
			if (0 == $entries[$i]->level) {
				$lastentrytop = $i;
				break;
			}
		}
        foreach ($entries as $i=>$entry) {
			...
			if ($level < 0) $text .= "\n<li class=\"first\">";
			elseif (isset($lastentrytop) AND $i == $lastentrytop) $text .= ($i == $lastofall) ? "\n<li class=\"last-top last-of-all\">" : "\n<li class=\"last-top\">";
			elseif ($i == $lastofall) $text .= "\n<li class=\"last-of-all\">";
			else $text .= "\n<li>";
