...
class aliroModuleHandler extends aliroCommonExtHandler {
	...
	protected function __construct () {
		...
		$database->addFieldIfMissing ('#__modules', 'authexclude', "tinyint(3) unsigned NOT NULL default '0' AFTER `exclude`");
	...
	private function getVisibleKeys ($position, $isAdmin) {
		...
		foreach ($exelements as $element) $this->addIfAuthorized ($result, $element);
		...
		if (isset($this->allModulesByMenu[$currentmenu])) {
			...
			foreach ($inelements as $element) $this->addIfAuthorized ($result, $element);
		}
	...
	private function addIfAuthorized (&$result, $element) {
		$module = $this->getModuleByID($element);
		if (aliroAuthoriser::getInstance()->checkUserPermission ('view', 'aliroModule', $element) == empty($module->authexclude)) $result[] = $element;
	}
...
class aliroModule extends aliroDatabaseRow {
	...
	public function renderModule ($area, $template, $i, $count) {
		...
		if (!is_object($modobject)) {
			trigger_error(sprintf(T_('Unable to instantiate %s'), $split[0]));
			return;
		}
		$activate = isset($split[1]) ? $split[1] : 'activate';
		if (!method_exists($modobject, $activate)) {
			trigger_error(sprintf(T_('Method %s not available for %s'), $activate, $split[0]));
			return;
		}
		try {
