...
final class aliroUserAuthenticator extends aliroAuthenticator {
	...
	public function systemLogin ($username=null, $passwd=null, $remember=null) {
		...
		if (is_object($my)) {
			...
			if ($database->defaultDate() == $this->lastVisit) {
				aliroCoreDatabase::getInstance()->doSQL("UPDATE #__core_users SET activation =  '' WHERE id = $my->id");
			...
			if ($remember) {
				...
				setcookie("usercookie[username]", base64_encode($username), $lifetime, "/");
				if (function_exists('mcrypt_encrypt') AND $my->id) {
					$coredatabase = aliroCoreDatabase::getInstance();
					$coredatabase->setQuery("SELECT salt FROM #__core_users WHERE id = $my->id");
					$salt = $coredatabase->loadResult();
					$iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_CBC);
					$iv = mcrypt_create_iv($iv_size, MCRYPT_RAND);
					if ($salt) $passwd = trim(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $salt, $passwd, MCRYPT_MODE_CBC, $iv));
					setcookie("usercookie[iv]", base64_encode($iv), $lifetime, "/");
				}
				setcookie("usercookie[password]", base64_encode($passwd), $lifetime, "/");
			}
		}
		else {
	...
	public function loginCheck ($username, $password, &$nogomessages, &$failmessages, $remember=false) {
		...
		foreach ($logresults as $result) {
			if ($result instanceof aliroLoginDetails) {
				...
				elseif (_ALIRO_LOGIN_GOOD == $result->status AND $result->user instanceof aliroCurrentUser) $my = $result->user;
			}
	...
	public function logout () {
		...
		if ($my->username) {
			$mambothandler = aliroMambotHandler::getInstance();
			$loginfo = new aliroLoginDetails($my->username);
	...
	public function authenticate (&$message, &$my, $username, $passwd, $remember=null) {
		$message = '';
		$database = aliroDatabase::getInstance();
		$my = new aliroCurrentUser();
		$escuser = $database->getEscaped($username);
		$database->setQuery( "SELECT * FROM #__users WHERE username='$escuser' OR email = '$escuser'");
		if ($database->loadObject($my)) {
			...
			$database->setQuery("SELECT activation FROM #__core_users WHERE id = $my->id  AND password=MD5(CONCAT(salt,'$escpass'))");
			$activations = $database->loadResultArray();
			if (is_array($activations) AND count($activations)) {
				unset($my->block);
				$this->lastVisit = $my->lastvisitDate;
				if ($activations[0]) {
					$database->doSQL("UPDATE #__core_users SET activation = '' WHERE id = $my->id");
					$this->activateLastVisit($my);
				}
				return true;
			}
			...
			if ($database->loadResult()) {
				...
				$database->doSQL("UPDATE #__core_users SET password = MD5(CONCAT('$salt', '$escpass')), salt = '$salt', activation = '' WHERE id = $my->id");
	...
	protected function activateLastVisit ($my) {
		aliroDatabase::getInstance()->doSQL("UPDATE #__users SET lastvisitDate = registerDate WHERE lastvisitDate < registerDate AND id = $my->id");
	}
}
