...
final class aliroMenuHandler extends cachedSingleton {
	...
	*/
    protected function __construct () {
        ...
        foreach ($menus as $key=>$menu) {
			...
			if (preg_match('/option=([^&]+)/', $menu->link, $matches)) {
				$this->byComponent[$matches[1]][] = $menu->id;
			}
        }
	...
	*/
    ...
    private function getIDLikeQuery ($query_items, $menuids, $published=false) {
    	$min = $ordering = 999999;
    	$result = 0;
        ...
        if (!$result AND isset($_SESSION['aliro_Itemid']) AND isset($this->menus[$_SESSION['aliro_Itemid']])) $result = $_SESSION['aliro_Itemid'];
        return $result;
    }
    ...
    public function matchURL ($published=true) {
    	...
    	elseif (isset($this->byComponent[$_REQUEST['option']])) {
	    	$query_items = $_SERVER['QUERY_STRING'] ? explode('&', $_SERVER['QUERY_STRING']) : array();
   			foreach ($_POST as $name=>$value) if (!is_array($value)) $query_items[] = $name.'='.$value;
	    	$link = $this->getIDLikeQuery($query_items, $this->byComponent[$_REQUEST['option']], $published);
