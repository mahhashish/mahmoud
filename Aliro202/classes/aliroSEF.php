...
class aliroSEF extends aliroFriendlyBase {
	...
	protected static $ignores = array('/forum');
	...
	private function __construct () {
		...
		$this->cache->setTimeout(_ALIRO_SEF_MAX_CACHE_TIME);
		...
		if (!$this->cached) {
			...
			$results = $this->database->doSQLget("SELECT u.*, m.htmltitle, m.robots, m.keywords, m.description FROM #__remosef_uri AS u LEFT JOIN #__remosef_metadata AS m ON u.id = m.id ORDER BY u.lastused DESC LIMIT {$this->config->buffer_size}");
			$this->addResultsToCache($results);
			unset($results);
			$results = aliroCoreDatabase::getInstance()->doSQLget("SELECT * FROM #__urilinks WHERE published != 0 ORDER BY LENGTH(uri) DESC");
			...
			foreach ($results as $result) {
				...
				if (!empty($result->isdefault)) $this->cached['DefaultLink'] = $prepared;
				else $this->cached['Links'][] = $prepared;
				unset($prepared);
			}
	...
	protected function addResultsToCache ($results) {
		foreach ($results as $result) if (!isset($this->cached['SEF'][$result->uri])) {
			$this->cached['ID'][$result->uri] = $result->id;
			$this->cached['SEF'][$result->uri] = $result->sef;
			$this->cached['Time'][$result->uri] = $result->refreshed;
			$metadata = $this->makeMetaObject($result);
			if ($metadata) $this->cached['Meta'][$result->uri] = $metadata;
		}
	}
	...
	public function nameForURL ($string) {
		...
		$words = explode($this->SEF_SPACE, $string);
		if (count($words) > $this->config->max_words) {
			$words = array_slice($words, 0, $this->config->max_words);
			$string = implode($this->SEF_SPACE, $words);
		}
		$string = $this->urlEncodeSome($string);
		return $string;
	}
	...
	public function despatcher () {
		...
		// if (strlen($uri) < 2 AND isset($this->cached['DefaultLink'])) return $this->prepareLinkData($this->cached['DefaultLink']);
		$menucount = aliroMenuHandler::getInstance()->getMenuCount();
		if (strlen($uri) > 1 OR 0 == aliroMenuHandler::getInstance()->getMenuCount()) {
		    $link = $this->getLinkByURI($uri);
		    if ($link) return $this->prepareLinkData($link);
		}
	    //$link = $this->getLinkByURI($uri);
	    //if ($link) return $this->prepareLinkData($link);
		//if (isset($this->cached['DefaultLink']) AND 0 == $menucount) return $this->prepareLinkData($this->cached['DefaultLink'], true);
		return array('', '', '', '', 0, null);
	}
	
	private function prepareLinkData ($link, $linkparm=null) {
		$results = explode(',', $link['class']);
		if (2 > count($results)) $results[] = '';
		return array(trim($results[0]), trim($results[1]), $link['notemplate'], $link['nohtml'], $link['id'], $linkparm);
	}
	...
	public function sefRetrieval() {
		...
		$postfix = strrchr($uri, '#');
		if ($postfix) $uri = substr($uri,0,-strlen($postfix));
		else $postfix = '';
		...
		$use_db = true;
		foreach (self::$ignores as $ignore) if ($ignore == substr($uri,0,strlen($ignore))) {
			$use_db = false;
		}
		if ($use_db) {
...
META_DATA;
	...
	public function sefRelToAbs ($string, $externalcall=true) {
		...
		$postfix = strrchr($string, '#');
		if ($postfix) $string = substr($string,0,-strlen($postfix));
		else $postfix = '';
		...
		if (isset($this->cached['SEF'][$string])) {
			if ((time() - $this->cached['Time'][$string]) < $this->config->cache_time) {
				return $this->live_site.$this->cached['SEF'][$string].$postfix;
			}
		}
		else {
			$fixstring = $this->database->getEscaped($string);
			$results = $this->database->doSQLget("SELECT u.*, m.htmltitle, m.robots, m.keywords, m.description FROM #__remosef_uri AS u LEFT JOIN #__remosef_metadata AS m ON u.id = m.id WHERE u.uri = '$fixstring' AND CRC32('$fixstring') = u.uri_crc ORDER BY u.refreshed DESC LIMIT 1");
			$this->addResultsToCache($results);
			if (isset($this->cached['SEF'][$string])) return $this->live_site.$this->cached['SEF'][$string].$postfix;
		}
		$oktasks = true;
		$option = $task = '';
	...
	private function outSubstitution ($inuri, $outuri, $external) {
		...
		foreach (self::$ignores as $ignore) if ($ignore == substr($finishedurl, 0, strlen($ignore))) return $finishedurl;
		...
		if (!isset($this->cached['SEF'][$inuri])) {
			$shortterm = $this->isTemporary($uri) ? '1' : '0';
			$this->database->doSQL("INSERT INTO #__remosef_uri (uri, uri_crc, sef, sef_crc, shortterm, lastused, refreshed, ipaddress) VALUES ('$uri', CRC32('$uri'), '$sef', CRC32('$sef'), $shortterm, $this->requestTime, $this->requestTime, '{$this->getIP()}')");
			$this->cached['ID'][$inuri] = $this->database->insertid();
			$this->database->doSQL("DELETE FROM #__remosef_uri WHERE uri = '$uri' AND refreshed != $this->requestTime");
		}
		elseif ($this->cached['SEF'][$inuri] != $finishedurl) {
			$this->database->doSQL("UPDATE #__remosef_uri SET sef = '$sef', sef_crc = CRC32('$sef'), lastused = $this->requestTime, refreshed = $this->requestTime, marker = 1 - marker, ipaddress = '{$this->getIP()}' WHERE uri_crc = CRC32('$uri') AND uri = '$uri'");
			$this->database->doSQL("DELETE FROM #__remosef_uri WHERE uri = '$uri' AND refreshed != $this->requestTime");
		}
		elseif (($this->requestTime - $this->cached['Time'][$inuri]) > (int) ($this->config->cache_time/5)) {
			$this->refreshed[$this->cached['ID'][$inuri]] = 1;
		}
	...
	private function isTemporary ($uri) {
		...
		return preg_match('/^option\=com_remository.*&chk\=/', $uri) ? true : false;
	}
	
	public function saveCache () {
		$sefcount = 0;
		if (count($this->refreshed)) {
			$updlist = implode(',', array_keys($this->refreshed));
			$this->database->doSQL("UPDATE #__remosef_uri SET lastused = $this->requestTime, ipaddress = '{$this->getIP()}' WHERE id IN ($updlist)");
			$this->refreshed = array();
		}
		if ($this->cachedirty) {
			$totalsaved = $this->cache->save($this->cached, 'sefDataURI', false);
			if (!$totalsaved) {
		...
		if (42 == mt_rand(0,199)) {
			$quarterago = $this->requestTime - 90*24*60*60;
			$this->database->doSQL("DELETE FROM #__remosef_uri WHERE refreshed < $quarterago");
		}
		elseif (42 == mt_rand(0,199)) {
			$weekago = $this->requestTime - 7*24*60*60;
			$this->database->setQuery("SELECT id FROM #__remosef_uri WHERE shortterm != 0");
			$ids = $this->database->loadResultArray();
			if (!empty($ids)) {
				$idlist = implode(',', $ids);
				$this->database->doSQL("DELETE FROM #__remosef_uri WHERE refreshed < $weekago AND id IN ($idlist)");
			}
		}
	}

	public function componentDetails ($params, $task='') {
		$string = ($task ? "task,$task/" : '');
		foreach ($params as $key=>$param) {
                    $param = $this->urlEncodeSome($param);
                    $string .= "$key,$param/";
                }
		return $string;
	}

	protected function urlEncodeSome ($string) {
		return preg_match('/[A-Za-z0-9,~_\.\-]*/', $string) ? $string : urlencode($string);
	}
}