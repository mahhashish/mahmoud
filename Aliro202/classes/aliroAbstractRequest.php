...
define ('_ALIRO_SPECIAL_ID_PAGE403', 200101);
define ('_ALIRO_SPECIAL_ID_PAGE404', 200102);
...
abstract class aliroAbstractRequest {
	...
	protected $needjquery = false;
	protected $needjqueryui = false;
	protected $needjqueryvalid = false;
	protected $validREST = false;
...
// The initial functions here are internal to Aliro, most are private or protected
	...
	protected function __construct () {
		...
		$this->basehref = $this->configuration->getCfg('live_site');
	...
	public function setValidREST () {
		$this->validREST = true;
	}
	
	public function isValidREST () {
	...
	public function instantiateSubclassOrClass () {
		$args = func_get_args();
		$className = array_shift($args);
		$base = aliro::getInstance();
		if ($base->classExists($className)) {
			$subclassName = aliro::getInstance()->firstSubClass($className);
			$r = new ReflectionClass($subclassName ? $subclassName : $className);
			return $r->newInstanceArgs($args);
		}
		else trigger_error(sprintf(T_('Invalid attempt to instantiate subclass or class, given %s as class name'),$className));
	}
    ...
    public function redirect ($url='', $message='', $severity=_ALIRO_ERROR_INFORM) {
        ...
        if ($message) $this->setInformationMessage ($message, $severity);
	...
	public function setInformationMessage ($message, $severity=_ALIRO_ERROR_INFORM) {
	...
	protected function commonHeaders () {
		...
		if (!empty($this->error404)) header ($_SERVER['SERVER_PROTOCOL'].' 404 Not Found');
    ...
    public function getItemid () {
		if ($this->special_itemid) return $this->special_itemid;
    	return $this->urilinkid ? 100000+$this->urilinkid : (isset($this->bestmatch) ? $this->bestmatch->id : 0);
    }
	
	public function getSpecialItemid () {
		return array(
			_ALIRO_SPECIAL_ID_PAGE403 => 'Page 403',
			_ALIRO_SPECIAL_ID_PAGE404 => 'Page 404',
		);
	}
	
	public function setSpecialItemid ($id) {
		$this->special_itemid = $id;
	}
...
SCRIPT_LINK;
	...
	public function addScriptText ($text, $position='default', $minify=true) {
	    if ($minify===true && !$this->getCfg('debug')) {
...
SCRIPT_LINK;
	...
	public function requireJquery ($withUI=false, $withValidate=false) {
		$this->needjquery = true;
		if ($withUI) $this->needjqueryui = true;
		if ($withValidate) $this->needjqueryvalid = true;
	}
    
	public function addCSS ($relativeFile, $media='screen', $notRelative=false) {
	    $site = $notRelative ? '' : $this->getCfg('live_site');
		$link = <<<CSS_LINK

	<link href="$site$relativeFile" rel="stylesheet" type="text/css" media="$media" />
...
CSS_LINK;
	...
	public function addCSSEmbed ($rawCSS, $position='head', $minify=true) {
		if ($minify===true && !$this->getCfg('debug')) {
    ...
    public function requestTooltip () {
    	if ($this->tooltip) return;
    	$this->tooltip = true;
    	
    	//Bring extra YUI modules to support overlays
		$yuiAdvOverlayReqs = array("overlay", "widget-position", "widget-position-ext");
		$aliroLoader = aliroResourceLoader::getInstance();
		$aliroLoader->addYUIModule($yuiAdvOverlayReqs);
    }
    ...
    public function showHead () {
        $html = "<base href=\"{$this->basehref}/\" />\r\n";
        $html .= aliroSEF::getInstance()->getHead($this->title, $this->metatags, $this->customtags);
		if ($this->needjquery) {
			$minified = $this->getCfg('debug') ? '' : '.min';
			$html .= <<<JQUERY_CDN
<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery$minified.js"></script>

JQUERY_CDN;
			
			if ($this->needjqueryui) {
				$this->addCSS("http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.16/themes/base/jquery-ui.css", 'screen', true);
				$html .= <<<JQUERY_UI_CDN
<script src="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.16/jquery-ui$minified.js"></script>
				
JQUERY_UI_CDN;

			}
			if ($this->needjqueryvalid) $html .= <<<JQUERY_VALID_CDN
<script src="http://ajax.microsoft.com/ajax/jquery.validate/1.7/jquery.validate$minified.js"></script>
...
GOOGLE_ANALYTICS;
    ...
    protected function invokeComponent ($menu=null) {
    	try {
			...
			elseif (is_object($menu)) $_SESSION['aliro_Itemid'] = $menu->id;
