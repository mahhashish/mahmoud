...
abstract class aliroComponentControllers extends aliroFriendlyBase {
	...
	public function makePageNav ($total, $togglescript=true) {
		$limit = min($total, $this->getUserStateFromRequest($this->option.'_page_limit', 'limit', intval($this->getCfg('list_limit'))));
		if ($togglescript) aliroHTML::getInstance()->toggleManyScript($limit);
