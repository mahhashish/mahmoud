...
class modulesAdminModules extends aliroComponentAdminControllers {
	...
	public function toolbar () {
		...
		switch ($this->task) {
			...
			case 'next':
				...
				$cur_template = aliroRequest::getInstance()->getTemplate();
				$toolbar->save();
				$toolbar->apply();
				$toolbar->cancel();
				break;
	...
	public function nextTask () {
		if ($this->codebase) {
			$extension = aliroExtensionHandler::getInstance()->getExtensionByID($this->codebase);
			$module = aliroModuleHandler::getInstance()->makeModuleFromExtension ($extension);
	...
	public function editTask ($row=null) {
		...
		if ( $row->checked_out AND $row->checked_out != $my->id ) {
			$this->setErrorMessage(sprintf(T_('The module %s is currently being edited by another administrator'), $row->module));
			$this->listTask();
			return;
		}
		...
		$lists['selections'] = aliroSelectors::getInstance()->menuLinks ($menus, true, true);
		...
		if ($extension) {
			...
			$params = aliroExtensionHandler::getInstance()->getParamsObject($row->params, $row->module);
		}
		else {
			$row->description = '';
	...
	private function commonSave () {
		...
		$row->incountry = $this->trimList($row->incountry);
		$row->excountry = $this->trimList($row->excountry);
		...
		if ($allmenus) {
			if ($row->exclude) $row->exclude = 0;
			else $database->doSQL("INSERT INTO `#__modules_menu` (moduleid, menuid) VALUES ($row->id, 0)");
		}
		elseif (empty($selectedmenus)) {
			if ($row->exclude) {
				$row->exclude = 0;
				$database->doSQL("INSERT INTO `#__modules_menu` (moduleid, menuid) VALUES ($row->id, 0)");
			}
		}
		else foreach ($selectedmenus as $menu) {
			$database->doSQL("INSERT INTO `#__modules_menu` (moduleid, menuid) VALUES ($row->id, $menu)");
		}
	...
	private function trimList ($string) {
	...
	public function applyTask () {
		...
		$this->redirect ( 'index.php?core='.$this->option.'&client='.$this->client .'&task=editA&hidemainmenu=0&id='.$row->id, $msg );
	}
