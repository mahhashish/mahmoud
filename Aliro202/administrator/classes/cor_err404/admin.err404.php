...
class err404AdminErr404 extends aliroComponentAdminControllers {
	...
	public function listTask () {
	    ...
	    $errors = $database->doSQLget("SELECT REPLACE(REPLACE(uri,'&amp;','&'),'&','&amp;') AS eluri, REPLACE(uri,'&amp;','&') AS details, timestamp, ip, errortype FROM #__error_404 ORDER BY timestamp DESC LIMIT {$this->pageNav->limitstart}, {$this->pageNav->limit}");
		foreach ($errors as $i=>$error) $errors[$i]->details = $link.urlencode($error->details);
	...
	public function editTask () {
		if ($this->errorid) {
			$database = aliroCoreDatabase::getInstance();
			$this->errorid = urldecode($this->errorid);
