...
MODULE_HTML;
    ...
    public function toolBarItemHTML ($task, $alt, $href, $icon, $iconOver, $linkIfJavaScript=true) {
		...
		$result = preg_match('#"(http://)*([^"]*)#', $icon, $matches);
		$iconfilename = isset($matches[2]) ? basename($matches[2]) : '';
        ...
        if ($linkIfJavaScript AND $this->request->getStickyAliroParam($_POST, 'alironoscript')) $startlink = $endlink = '';
            ...
            <a id="{$task}__toolbarLink" class="toolbar" href="$href">
LINK_START;
            $endlink = '</a>';
        }
        $html = <<<SHORT_ITEM
        
        <div class="toolitem $iconclass">
            $startlink
            $alt
            $endlink
      	...
      	<form action="index.php" method="post" name="loginForm" id="loginForm">
			<div id='AliroLoginBody'>
				...
				<div id='AliroUserName'>
					...
					<input id='AliroAdminUser' id='usrname' name='usrname' type='text' size='15' />
				</div>
				<div id='AliroPassword'>
					<label for='AliroPass'>Password:</label>
					<input id='AliroPass' id='pass 'name='pass' type='password' size='15' />
				</div>
				<div id="wrapper2">
