...
class Pages extends CI_Controller {
    ...
    public function getUserIP() {
        ...
        return $this->pagesmodel->checkifIPexists($visitorIP);
    }

    public function index() {
        $this->page_info['hh'] = $this->getUserIP();
    ...
    public function section($pageSlug, $offset = 1) {
        if (!empty($pageSlug)) {
            $uri_segment = 4;
            ...
            if (is_numeric($offset) && $offset >= 1) {
                    ...
                    $config['first_url'] = base_url() . 'pages/section/' . $pageSlug . '/1';
                    ...
                    $this->load->view('mainui/tpl_home/sidebar', $this->pageLinks);
                    $this->sectionPosts['categ_news'] = $this->pagesmodel->allpost_from_category($pageSlug, $config['per_page'], ($offset - 1) * ($config['per_page']));
