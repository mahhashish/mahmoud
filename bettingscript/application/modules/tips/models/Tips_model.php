...
class Tips_model extends MY_Model
{
	...
	//-----------------------------------------------	
	public function get_frends_activities($following_id,$limit = array())
	{
        $this->db->order_by('created_on', 'desc');
		$this->db->where('status', 2);
	...
	public function get_comments($id){
		$this->db->select("display_name,avatar,comment,created_at")
		->from('comments')
		->join($this->user_table, $this->user_table . '.id = ' .  'comments.user_id', 'left')
		->where('comments.preview_id =' , $id) //exclude admin role
	...
	public function save_comment($id){
		...
		if($comment){
			...
			$this->db->insert('comments', array('user_id'=>$this->session->user_id,'preview_id'=>$id,'comment'=> $comment));
		}
	}
}