...
///////////////////////////////////////////////////////		
	...
	public function profile($user_id,$pagination= false, $page=0)
    {
		...
		if ($user_id) {
			$followings = $this->tipsters_model->get_followings($user_id);
			$following_ids = array();
if($followings){
	
			foreach ($followings as $r) {
			$following_ids[] = $r->following_id;
			}
			$tips =  $this->tips_model->get_frends_activities($following_ids);
		

			$this->load->library('pagination');

			$per_page = 20;
			$this->pager['base_url']    = base_url().'/tipsters/profile/'.$user->id.'/page';
			$this->pager['per_page'] = $per_page;
			$this->pager['total_rows']  = count($tips);
			$this->pager['uri_segment'] = 5;
			$this->pagination->initialize($this->pager);
		$tips =  $this->tips_model->get_frends_activities($following_ids,array($per_page,$page));
}
		}
