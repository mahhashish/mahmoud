...
class Users extends Front_Controller
{
	...
	public function fblogin(){
		...
		$this->load->library('facebook', array(
           'appId' => '1078343032223903',
            'secret' => '1244e5c752e58002b81b74cde494da45',
            ));
        ...
        if ($user) {
            try {
                $fbdata= $this->facebook->api('/me?locale=en_US&fields=name,email,id,location,gender');
				
				
				$str = strtolower($fbdata['name']);
				$username = str_replace(' ', '', $str);
				$displayname = substr($username, 0, 12);
				
				if (true === $this->auth->login($username,$fbdata['id']) ) 
				{
					log_activity(
					$this->auth->user_id(),
					...
					$fullname=explode(" ",$fbdata['name']);
	
					//getting firstname and lastname from the full name
					$firstname=$fullname[0];
					$lastname=$fullname[1];
					
					$social_picture="https://graph.facebook.com/".$fbdata['id']."/picture";
					$data=array('role_id'=>3,'username'=>$username,'display_name'=>$displayname,'first_name'=>$firstname,'last_name'=>$lastname,'email'=>$email,'social_avatar'=>$social_picture,
					'last_login'=>$timestamp,'created_on'=>$timestamp,
					'active'=>1,'deleted'=>0,'banned'=>0
					);
					
					$password = $this->auth->hash_password($fbdata['id']);
					$data['password_hash'] = $password['hash'];
					$id=$this->user_model->fbinsert($data);
            ...
            $loginUrl = $this->facebook->getLoginUrl(array(
                ...
                'scope' => array('email','user_location') // permissions here
            ));
