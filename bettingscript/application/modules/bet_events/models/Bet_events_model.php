...
class Bet_events_model extends MY_Model
{
	...
	public function get_home_odds($home_team,$match_id)
	{

		$this->db->where('choice_name',$home_team);
		$this->db->where('match_id',$match_id);
		$this->db->where_in('bet_name',array('Match Result','Match Winner','Match Winner 2','Winner','Tournament Match Bet'));
	...
	public function get_away_odds($away_team,$match_id)
	{

		$this->db->where('choice_name',$away_team);
		$this->db->where('match_id',$match_id);
		$this->db->where_in('bet_name',array('Match Result','Match Winner','Match Winner 2','Winner','Tournament Match Bet'));
	...
	public function get_draw_odds($match_id)
	{

		$this->db->where('choice_name','Draw');
		$this->db->where('match_id',$match_id);
		$this->db->where_in('bet_name',array('Match Result','Match Winner','Match Winner 2','Winner'));
	...
	public function get_home_bet_id($home_team,$match_id)
	{
		
		$this->db->select('bet_id');
		$this->db->where('choice_name',$home_team);
		$this->db->where('match_id',$match_id);
		$this->db->where_in('bet_name',array('Match Result','Match Winner','Match Winner 2','Tournament Match Bet'));
	...
	public function get_home_choice_id($home_team,$match_id)
	{
	...
	public function get_draw_bet_id($match_id)
	{
	...
	public function get_draw_choice_id($match_id)
	{
	...
	public function get_away_bet_id($away_team,$match_id)
	{
	...
	public function get_away_choice_id($away_team,$match_id)
	{
	...
	public function get_outright_bets($league_id)
	{
		...
		->where_in('bet_name', array('Outright Winner','Winner'))
		->order_by('odd','asc');
		$query = $this->db->get('bets');
...
/////////////////////////////////////////////////////////////	
	public function set_update()
	{
		...
		$query = $this->db->update('last_update');
							...
							foreach ($match as $bets) 
							{
								foreach ($bets as $bet) 
								{
									...
									$bet_id = $bet->attributes()->id;
										...
										$away_team = substr($match_name, strpos($match_name, " - ") +2); 
										$away = str_replace(' and ',' & ', $away_team);
										
										$str = explode(" - ", $match_name, 2);
										$home_team = $str[0];
										$home = str_replace(' and ',' & ', $home_team);
										...
										foreach ($bet as $choice) 
										{
											...
											$this->db->select('bet_name,choice_name,match_id')
