...
abstract class aliroDBGeneralRow {
	...
	public function bind( $array, $ignore='', $strip=true ) {
		...
		return $this->bindDoWork ($array, $fields, $strip);
	}

	public function bindOnly ($array, $accept='', $strip=true) {
		$fields = $this->getDatabase()->getAllFieldNames ($this->tableName);
		foreach ($fields as $key=>$field) if (false === strpos($accept, $field)) unset($fields[$key]);
		return $this->bindDoWork ($array, $fields, $strip);
	}

	private function bindDoWork ($array, $fields, $strip) {
		if (is_array($array)) {
			$request = aliroRequest::getInstance();
			foreach ($fields as $field) if (isset($array[$field]) AND is_string($array[$field])) {
				$this->$field = $strip ? $this->stripMagicQuotes($array[$field]) : $array[$field];
				if ('params' != $field AND (false !== strpos($this->$field, '&') OR false !== strpos($this->$field, '<'))) {
					$this->$field = $request->doPurify($this->$field);
				}
			}
			return true;
		}
		$this->_error = strtolower(get_class($this)).T_('::bind failed, parameter not an array');
		return false;
	}
	...
	public function lacks( $property ) {
		...
		$this->_error = sprintf (T_('WARNING: %s does not support %s.'), get_class($this), $property);
		return true;
	}
	...
	function checkout( $who, $key=null ) {
...
abstract class mosDBTable extends aliroDBGeneralRow {
	...
	function __call ($method, $args) {
		...
		else trigger_error(T_('Invalid method call to mosDBTable'));
	}
	...
	function hit( $keyvalue=null ) {
		...
		if (aliroRequest::getInstance()->getCfg('enable_log_items')) {
			$now = date( "Y-m-d" );
...
abstract class aliroDatabaseRow extends aliroDBGeneralRow {
	...
	function __get ($property) {
		...
		if (!is_object($field)) trigger_error(T_('Database row attempt to obtain invalid property: ').$property);
