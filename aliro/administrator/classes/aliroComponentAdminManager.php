...
class aliroComponentAdminManager extends aliroComponentManager {
	...
	public function __construct ($component, $system, $version) {
		...
		$this->cid = $this->getParam($_REQUEST, 'cid', array(0));
		if (is_array( $this->cid )) {
			foreach ($this->cid as &$value) $value = intval($value);
			$this->currid=$this->cid[0];
		}
		else $this->currid = intval($this->cid);
...
abstract class aliroComponentAdminControllers extends aliroComponentControllers {
	...
	protected function __construct ($manager) {
		...
		$this->act = $manager->act;
		$this->task = $manager->task;
		$this->cid = $manager->cid;
		$this->optionurl .= $this->option.'&amp;act='.$this->act;
