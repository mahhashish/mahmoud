...
class foldersAdminFolders extends aliroComponentAdminControllers {
	...
	public function editTask () {
		...
		$handler = aliroFolderHandler::getInstance();
		$folder = $handler->getFolder($this->manager->currid);
	...
	public function saveTask () {
	    ...
	    if ($this->manager->currid == 0) $folder->published = 1;
	...
	public function deleteTask () {
		...
		$this->manager->check_selection(T_('Please mark item(s) for deletion'));
	...
	private function publishToggle ($publish) {
		...
		$this->manager->check_selection(_DOWN_PUB_PROMPT.($publish ? 'publish' : 'unpublish'));
	    aliroFolder::togglePublished($this->manager->cid,$publish);
